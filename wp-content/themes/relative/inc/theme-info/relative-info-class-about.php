 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'relative_info_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class relative_info_page {
		/**
		 * Define the version of the class.
		 * @var string $version The relative_info_page class version.
		 */
		...
		/**
		 * The single instance of relative_info_page
		 * @var relative_info_page $instance The  relative_info_page instance.
		 */
		private static $instance;

		/**
		 * The Main relative_info_page instance.
		 * We make sure that only one instance of relative_info_page exists in the memory at one time.
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof relative_info_page ) ) {
				self::$instance = new relative_info_page;
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', 'relative-welcome', array(
					$this,
					'page_render',
				) );
			}
		...
		public function welcome_admin_notice() {
			...
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'relative_notice_welcome' ) && 'themes.php' == $pagenow ) {

					echo '<div class="relative-info-about updated notice is-dismissible">';
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('relative-hide-notice', 'welcome')), 'relative_hide_notices_nonce', '_relative_notice_nonce')) . '" style="z-index: 0;padding: 10px;text-decoration: none;" >';
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'relative'). '</span>';
					echo '</a>';
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $this->tabs ) ) {
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'themes.php?page=relative-welcome' ) ) . '&tab=' . esc_attr( $tab_key ) . '" class="nav-tab ' . ( $active_tab == $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		*/
		public function theme_review() {
			echo '<div id="relative-support-content" class="feature-section two-col">';
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
					...
					echo '<div id="relative-getting-started" class="feature-section three-col">';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					echo '<div id="relative-free-pro" class="feature-section">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="relative-free-pro-button">';
						echo '<td></td>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			// enqueue css files
			if ( 'themes.php' === $pagenow || 'appearance_page_relative-welcome' == $hook_suffix ) {
				wp_enqueue_style( 'relative-about-page-css', get_template_directory_uri() . '/inc/theme-info/css/relative-info-backend.css' );
			}

			// enqueue js files
			if ( 'appearance_page_relative-welcome' == $hook_suffix ) {
				wp_enqueue_script( 'relative-about-page-js', ( get_template_directory_uri() . '/inc/theme-info/js/relative-info-backend.js' ), array( 'jquery' ), '', 'true' );
			}
		...
		public function hide_notice() {
			if (isset($_GET['relative-hide-notice']) && isset($_GET['_relative_notice_nonce'])) {
				if (!wp_verify_nonce($_GET['_relative_notice_nonce'], 'relative_hide_notices_nonce')) {
					wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'relative'));
				}
				if (!current_user_can('edit_theme_options')) {
					wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'relative'));
				}
				$hide_notice = sanitize_text_field( wp_unslash($_GET['relative-hide-notice']));
				update_option('relative_notice_' . $hide_notice, 1);
			}
		}
